 ...
 * Generates realistic sample data for entities based on their fields
 */
...
/**
 * Updates all entities in a list with sample data
 */
export function updateEntitiesWithSampleData(
  entities: Entity[],
  count: number = 3
): Entity[] {
  return entities.map((entity) => updateEntityWithSampleData(entity, count));
}
